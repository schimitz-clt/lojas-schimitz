...
import { HANDOFF_MESSAGE, PIX_DISCOUNT_PCT, FREE_SHIPPING_ABOVE } from './chat.facts';
...
assert.ok(faqShip && faqShip.includes('299'));
...
assert.ok(/299|PIX|12x|SCHIMITZ/i.test(fallbackNoKey));
...
assert.equal(FREE_SHIPPING_ABOVE, 299);
