...
| Pagamento Mercado Pago | SCH-003 — Payments API + Card Brick + PIX (não Checkout Pro) |
| Frete | SCH-004 — regras CEP / carrier `propria` (Melhor Envio ainda stub) |
| Admin profissional | SCH-005 — JWT + role admin (+ Notificações push FCM) |
...
Venda paga: loja sempre recebe in-app **Novo pagamento** + e-mail **Nova venda paga** (todos os admins ativos, mesmo se o comprador for admin); e-mail inclui `wa.me` click-to-chat.
Marketplace: `docs/MARKETPLACE.md`. Split MP (não ligado): `docs/MARKETPLACE-MP-SPLIT-PLAN.md`. Chat IA: widget em http://localhost:3000 e POST /api/v1/chat. Env opcional OPENAI_API_KEY, CHAT_API_BASE, CHAT_MODEL. Sem chave = FAQ + WhatsApp. Ver docs/CHAT.md.
